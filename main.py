...
class Stream:
    ...
    def m_init(self, ):
        ...
        header_ = ['类型', 'sequence或ts_name', 'ttfb', 'ttlb']
        ...
        while self.ts_urls != []:
            ...
            if not os.path.exists(ts_path):
                ...
                if r_b is not None:
                    ...
                    self.ts_tt_v.append([ts_ttfb, ts_ttlb])

                    logger.info('{} ts_success：{}'.format(self.stream_name, self.ts_url))

                else:
                    logger.error('"process_m3u8" "if r_b is None" ，"get_context" 结果为空; url: {}'.format(self.url_m3))
                    self.ts_tt_v.append([0, 0])
                    return
            # time.sleep(1)
    ...
    def process_delay(self):
        ...
        # 每次循环 m3u8 必有，所以直接执行
        time_l.append(['m3u8', self.sequence, self.m3_ttfb, self.m3_ttlb])
        # 判断有没有执行过ts，直到队列取完
        while self.ts_tt_k:
